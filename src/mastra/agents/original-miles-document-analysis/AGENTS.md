# AGENTS.md — original-miles-document-analysis
...
- Não roda como `outputProcessor` de nada e não tem relação com o guardrail ou a working memory — é só uma etapa de normalização de input, junto com `agents/original-miles-image-analysis/`.
- Compartilha a factory `agents/shared/media-analysis-agent.ts` com `agents/original-miles-image-analysis/` — os dois agentes só diferem no tipo da parte multimodal (`file` vs `image`) e no prompt.

## Arquivos desta pasta

- `original-miles-document-analysis-agent.ts` — monta o `Agent` via `createMediaAnalysisAgent` e exporta `analyzeDocument(mediaUrl, mediaType, userMessage)`.
